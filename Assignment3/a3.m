...
function ret = loss(model, data, wd_coefficient)
  ...
  % <number of inputs i.e. 256> by <number of data cases>. Each column describes a different data case. 
  % data.targets is a matrix of size 
  % <number of classes i.e. 10> by <number of data cases>. Each column describes a different data case. 
...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  P_h1 = logistic(model.input_to_hid * data.inputs);
  Z = model.hid_to_class * P_h1; % SHOULD be softmax
  w = sum(exp(Z), 1);
  k = size(data.targets, 1);
  w = repmat(w, k, 1);
  w = exp(Z) ./ w;
  E_by_a1 = (w-data.targets) ;
  d_hidden = (E_by_a1) * P_h1';
  E_by_h1 = model.hid_to_class' * (E_by_a1); 
  h1_by_a2 = P_h1 .* (1-P_h1);
  d_input =  (E_by_h1 .* h1_by_a2) * data.inputs';
  ret.input_to_hid = model.input_to_hid * wd_coefficient + d_input /m;
  ret.hid_to_class = model.hid_to_class * wd_coefficient + d_hidden /m;
  %}
  ...
  % size: <number of classes, i.e. 10> by <number of data cases>
  class_prob = exp(log_class_prob);
  % the sum of error made by output minus target
  % size: <number of classes, i.e. 10> by <number of data cases>
  E_output_target = class_prob - data.targets;
  % the sum of derivative from error to the weights in the hidden layer
  % size: <number of classes i.e. 10> by <number of hidden units>
  d_hid_to_class = E_output_target * hid_output';
  % the sum of error made from hidden layer to output layer
  % size: <number of hidden units> by <number of data cases>
  E_hid_output = model.hid_to_class' * E_output_target;
  % the intermediate produced by the derivative of logistic units in the hidden layer
  % size: <number of hidden units> by <number of data cases>
  d_input_to_hid = hid_output .* (1 - hid_output);
  % size: <number of hidden units> by <number of inputs i.e. 256>
  d_input_to_hid = (E_hid_output.* d_input_to_hid) * data.inputs';
  
  m = size(data.inputs, 2);  
  ret.input_to_hid = model.input_to_hid * wd_coefficient + d_input_to_hid / m;
  ret.hid_to_class = model.hid_to_class * wd_coefficient + d_hid_to_class / m;
